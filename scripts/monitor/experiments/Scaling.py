...
class Scaling:
    ...
    def scale_and_wait(self, tm_name, replicas):
        ...
        self.__log.info(f"Waiting for {self.interval_scaling_s} seconds")
    ...
    def scale_operator_linear(self, number, tm_type):
        ...
        self.__log.info(f"Current number of taskmanagers: {taskmanagers_count_dict}")
    ...
    def scale_operator_exponential(self, number, tm_type):
        ...
        self.__log.info(f"Scaling sequence: {scaline_sequence}")
    ...
    def scale_op(self, taskmanager):
        ...
        match scaling_method:
            ...
            case _:
                self.__log.warning(
                    f"Scaling method {scaling_method} not supported. Defaulting to linear."
                )
                self.scale_operator_linear(number, tm_type)
    ...
    def scale_step(self, step):
        ...
        self.__log.info(f"Scaling on node : {node}")
    ...
    def run(self):
        self.__log.info("First taskmanager, just waiting")
        sleep(self.interval_scaling_s)
        self.setup_run()
        self.__log.info("Scaling started.")
        for i in range(len(self.steps)):
            try:
                if i > 0:
                    ...
                    if next_node:
                        self.__log.info(f"Next node: {next_node}")
                        # Mark this node with schedulable
                        self.k.node_manager.mark_node_as_schedulable(next_node)
                    else:
                        self.__log.error("No more nodes available.")
                        break
            except Exception as e:
                self.__log.error(f"Error while getting next node: {e}")
                break

            # Scale step
            self.scale_step(i)
            sleep(5)
        self.__log.info("Scaling finished.")

    def setup_run(self):
        self.__log.info("Setting up experiment.")
        ...
        if self.steps[0]["taskmanager"][0]["number"] == 0:
            ...
            self.log.info("First taskmanager scaled, removing from strategy.")
        if len(self.steps[0]["taskmanager"]) == 0:
            self.steps.pop(0)
            self.log.info("First node scaled, removing from strategy.")
        if len(self.steps) == 0:
            self.log.info("No more steps to scale.")
