...
- **dockerfile**: Contains the Dockerfile for setting up the development environment **scalehub**.
...
- **experiment-monitor**: Contains the Python script that handles experiments from within the cluster.
...
During reservation of G5k nodes (with `shub provision`), the playbook **cluster-setup-{kubernete-flavour}.yaml** is executed and it sets up kubernetes on the reserved nodes.

The playbook **storage** deploys the NFS plugin for long-term storage access and MinIo as a distributed store for Apache Flink.

The playbook **network** deploys ingress resources that expose monitoring dashboards.
