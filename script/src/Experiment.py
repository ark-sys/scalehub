...
class ExperimentData:
    ...
    def __init__(self, log: Logger, exp_path: str, start_ts: int, end_ts: int, db_url: str):
        self.__log = log
    ...
    def export_experiment_data(self):
        ...
        if tpt_timeseries_path is None or par_timeseries_path is None:
            ...
            if df1.empty or df2.empty:
                self.__log.error("One or both exported CSVs appear to be empty. Failed to join.")
                exit(1)
            else:
    ...
    def eval_stats(self, skip_duration):
        ...
        try:
            ...
            df = df.groupby('Parallelism').apply(
                lambda x: x[x['Time'] >= x['Time'].min() + pd.Timedelta(seconds=skip_duration)]).reset_index(drop=True)
    ...
    def eval_plot(self, stats):
        ...
        ax.errorbar(stats.index, stats['mean'], yerr=stats['std'], fmt='o', linestyle='-', capsize=4)

        # Set title and axis labels
        _, date, n = self.exp_path.split('/')[-3:]
        plot_title = "Date: {} N: {}".format(date, n)
        ax.set_title(plot_title)
        ax.set(xlabel='Parallelism', ylabel='Throughput', xticks=list(range(1, len(stats.index) + 1)))

        # Parse log file for experiment info
        log_path=os.path.join(self.exp_path,"exp_log.txt")
        job_name, num_sensors_sum, avg_interval_ms, start_ts, end_ts = self.parse_log(log_path)
    ...
    def parse_log(self, log_path: str):
        ...
        lg_matches = re.finditer(r'LG : (.+?)\s+topic : (.+?)\s+num_sensors : (\d+)\s+interval_ms : (\d+)', logs,
                                 re.DOTALL)
        ...
        return job_name, num_sensors_sum, avg_interval_ms, start_timestamp, end_timestamp
...
class Experiment:
    ...
    def start_experiment(self):
        ...
        with open(self.log_file, "w") as file:
            ...
            file.write(f"    max parallelism : {self.config.get_int(Key.TRANSCCALE_PAR)}\n")
            file.write(f"    interval : {self.config.get_int(Key.TRANSSCALE_INTERVAL)}\n")
            file.write(f"    warmup : {self.config.get_int(Key.TRANSSCALE_WARMUP)}\n")
    ...
    def end_experiment(self):
        ...
        data: ExperimentData = ExperimentData(log=self.__log, exp_path=self.exp_path, start_ts=self.start_ts,
                                              end_ts=self.end_ts, db_url=self.db_url)
    ...
    def full_run(self):
        ...
        # Launch Flink Job
        self.m.run_command(pod_name="flink-jobmanager",
