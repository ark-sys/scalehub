...
from src.scalehub.platforms.EnosPlatform import (
    EnosPlatform,
...
@patch("src.scalehub.platforms.EnosPlatform.subprocess.check_output")
def test_enosplatform_estimate_required_nodes(mock_subprocess, valid_config, logger_mock):
    ...
    mock_subprocess.return_value = (
        b'{"items": [{"architecture": {"nb_cores": 16}, "main_memory": {"ram_size": 134217728}}]}'
    )
    platform = EnosPlatform(log=logger_mock, platform_config=valid_config)

    vm_groups = [
        VMGroup(role="producers", conf={"core_per_vm": 4, "memory_per_vm": 8192}, count=2),
        VMGroup(role="consumers", conf={"core_per_vm": 2, "memory_per_vm": 4096}, count=3),
    ]
...
@patch("src.scalehub.platforms.EnosPlatform.en.init_logging")
@patch("src.scalehub.platforms.EnosPlatform.EnosPlatform._create_base_config")
